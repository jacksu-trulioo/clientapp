...
function MarketUpdatesScreen() {
  ...
  const { data, error } = useSWR<InsightsWithPagination>(
    [`/api/portfolio/insights/market-updates?currentPage=${currentPage}`, lang],
    (url, lang) =>
      fetch(url, {
        headers: {
    ...
    >
      <Container as="section" maxW="full" p="0">
        ...
        <Text
          ...
          textAlign={{ base: "center", md: "start" }}
        >
          {t("page.marketUpdates.subheading")}
        </Text>
        <>
...
export default withPageAuthRequired(MarketUpdatesScreen)
