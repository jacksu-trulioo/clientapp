...
function WhitepapersScreen() {
  ...
  const { data, error } = useSWR<InsightsWithPagination>(
    [`/api/portfolio/insights/whitepapers?currentPage=${currentPage}`, lang],
    (url, selectedLang) =>
      fetch(url, {
        headers: {
    ...
    >
      <Container as="section" maxW="full" p="0">
        ...
        <Text
          ...
          textAlign={{ base: "center", md: "start" }}
        >
          {t("page.whitepapers.subheading")}
        </Text>
        <>
          {data?.data
            ?.slice((currentPage - 1) * pageLimit, pageLimit * currentPage)
            .map((insight: Insight) => {
              return (
                <div key={insight.id}>
                  <InsightCard
...
export default withPageAuthRequired(WhitepapersScreen)
