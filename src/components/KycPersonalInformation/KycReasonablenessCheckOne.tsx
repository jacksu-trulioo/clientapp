...
import { useKycReasonablenessCheckOneSchema } from "./KycPersonalInformation.schema"
...
const KycPepCheck = React.forwardRef<HTMLDivElement, unknown>(
  function KycPepCheck(_props, _ref) {
    ...
    const validationSchema = useKycReasonablenessCheckOneSchema()
    const checkList = Object.keys(AddressNotMatchTaxResidence)
      .filter((key) => isNaN(Number(key)))
      .map((key) => ({
        key: key,
        label: t(
          `kyc:personalInformation.kycReasonablenessCheck.radio.addressNotMatchTaxResidence.options.${key}`,
        ),
        value:
          AddressNotMatchTaxResidence[
            key as keyof typeof AddressNotMatchTaxResidence
          ],
      }))
              ...
              <Form style={{ width: "100%" }}>
                <Stack isInline alignItems="flex-end" mb="7">
                  <Text fontSize="sm" color="gray.400">
                    {t(
                      "personalInformation.kycReasonablenessCheck.radio.addressNotMatchTaxResidence.label",
                    )}
                  </Text>
                </Stack>
                <VStack spacing={["6", "8"]} alignItems="start">
                  <RadioGroupControl
                    name="reasonablenessCheck.addressNotMatchTaxResidence.checkValue"
                    onChange={async (
                      valueChangeEvent: React.ChangeEvent<HTMLInputElement>,
                    ) => {
                      await formikProps.setFieldValue(
                        "reasonablenessCheck.addressNotMatchTaxResidence.checkValue",
                ...
                {formikProps.values.reasonablenessCheck
                  ...
                  <>
                    <InputControl
                      name="reasonablenessCheck.addressNotMatchTaxResidence.otherText"
                      inputProps={{
                        placeholder: t(
