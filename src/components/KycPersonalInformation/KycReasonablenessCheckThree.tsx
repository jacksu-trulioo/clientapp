...
import { useKycReasonablenessCheckThreeSchema } from "./KycPersonalInformation.schema"
...
const KycPepCheck = React.forwardRef<HTMLDivElement, unknown>(
  function KycPepCheck(_props, _ref) {
    ...
    const validationSchema = useKycReasonablenessCheckThreeSchema()
    const checkList = Object.keys(NationalityNotMatchTaxResidence)
      .filter((key) => isNaN(Number(key)))
      .map((key) => ({
        key: key,
        label: t(
          `kyc:personalInformation.kycReasonablenessCheck.radio.nationalityNotMatchTaxResidence.options.${key}`,
        ),
        value:
          NationalityNotMatchTaxResidence[
            key as keyof typeof NationalityNotMatchTaxResidence
          ],
      }))
              ...
              <Form style={{ width: "100%" }}>
                <Stack isInline alignItems="flex-end" mb="7">
                  <Text fontSize="sm" color="gray.400">
                    {t(
                      "personalInformation.kycReasonablenessCheck.radio.nationalityNotMatchTaxResidence.label",
                    )}
                  </Text>
                </Stack>
                <VStack spacing={["6", "8"]} alignItems="start">
                  <RadioGroupControl
                    name="reasonablenessCheck.nationalityNotMatchTaxResidence.checkValue"
                    onChange={async (
                      valueChangeEvent: React.ChangeEvent<HTMLInputElement>,
                    ) => {
                      await formikProps.setFieldValue(
                        "reasonablenessCheck.nationalityNotMatchTaxResidence.checkValue",
                        valueChangeEvent?.target?.value,
                      )
                      if (
                        valueChangeEvent?.target?.value ===
                        NationalityNotMatchTaxResidence.NationalityNotMatchTaxResidence4
                      ) {
                        await formikProps.setFieldValue(
                ...
                {formikProps.values.reasonablenessCheck
                  ...
                  <>
                    <InputControl
                      name="reasonablenessCheck.nationalityNotMatchTaxResidence.otherText"
                      inputProps={{
                        placeholder: t(
